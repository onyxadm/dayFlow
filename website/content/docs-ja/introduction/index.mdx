...
/* DayFlow: DayFlow コンポーネント内で使用されているユーティリティクラスを Tailwind が生成できるように、コンパイル済みの JS をスキャンします */
@source '../node_modules/@dayflow/core/dist/**/*.js';
@source '../node_modules/@dayflow/plugin-drag/dist/**/*.js'; /* ドラッグプラグインを使用する場合 */
@source '../node_modules/@dayflow/plugin-sidebar/dist/**/*.js'; /* サイドバープラグインを使用する場合 */
```
...
> **ヒント:** `create-dayflow` セットアップウィザードを使用し、Tailwind CSS に「はい」を選択した場合、これらの設定はルート CSS ファイルに自動的に追加されます。
