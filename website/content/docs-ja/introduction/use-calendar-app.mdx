...
| `allDaySortComparator`      | `AllDaySortComparator`      | 任意 | カレンダーグループ順 | 全ビューでの終日イベントの行順を制御するカスタム比較関数。                              |
...
- `true`：すべての変更操作（ドラッグ、作成、編集）を無効にします。
- `ReadOnlyConfig`：詳細制御：
...
全ビュー（日・週・月・年）での終日イベントの行順を制御します。デフォルトでは `calendarId` でグループ化（初出現順）され、グループ内は元の読み込み順が保持されます。
...
**組み込みプリセット**（`@dayflow/core` からエクスポート）：

| プリセット             | 動作                                                                                 |
| :--------------------- | :----------------------------------------------------------------------------------- |
| `sortAllDayByCalendar` | `calendarId` のアルファベット順でグループ化。`calendarId` がないイベントが先頭に来る |
