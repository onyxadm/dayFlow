...
| `allDaySortComparator`      | `AllDaySortComparator`      | 可选 | 按日历分组顺序      | 控制所有视图中全天事件的行排列顺序的自定义比较函数                    |
...
- 与 `DayFlowCalendar` 上的 `customDetailPanelContent` 或 `customEventDetailDialog` 配合使用，可替换 UI 同时保留内部状态机。

### useCalendarHeader

- `true`（默认）：渲染内置日历头部。
- `false`：完全隐藏头部。
- `(props) => ReactNode`：渲染自定义头部，详见 [Calendar Header](/docs-zh/features/calendar-header)。

### readOnly

- `true`：禁用所有修改操作（拖拽、创建、编辑）。
...
控制所有视图（日、周、月、年）中全天事件的行排列顺序。默认情况下，事件按 `calendarId` 分组（首次出现顺序），组内保留原始加载顺序。
...
**内置预设**（从 `@dayflow/core` 导出）：

| 预设                   | 行为                                                           |
| :--------------------- | :------------------------------------------------------------- |
| `sortAllDayByCalendar` | 按 `calendarId` 字母顺序分组；没有 `calendarId` 的事件排在最前 |

```tsx
import { sortAllDayByCalendar } from '@dayflow/core';

const calendar = useCalendarApp({
  allDaySortComparator: sortAllDayByCalendar,
});
```
...
```tsx
calendar.app.updateConfig({ allDaySortComparator: sortAllDayByCalendar });
