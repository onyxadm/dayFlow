...
| `allDaySortComparator`      | `AllDaySortComparator`      | Optional | Calendar-group order   | Custom comparator controlling the row order of all-day events across all views.                                    |
...
Controls the row order of all-day events in every view (day, week, month, year). By default, events are grouped by their `calendarId` in first-seen order; within each group the original load order is preserved.
...
**Built-in preset** exported from `@dayflow/core`:

| Preset                 | Behavior                                                                               |
| :--------------------- | :------------------------------------------------------------------------------------- |
| `sortAllDayByCalendar` | Groups events by `calendarId` alphabetically; events without a `calendarId` sort first |
...
// Restore default (first-seen calendar-grouping) order
